...
class CODI(torch.nn.Module):
    ...
    def compute_adaptive_loss(
        ...
        核心思路：
        1. 教师前向 → 基于熵识别Latent区域边界
        2. 学生在每个Latent区域入口决定循环次数
        3. 对齐每个Latent区域出口的hidden state
        
        流程示意:
            教师: E E E [L L L L] E E [L L] E E E
                        ^     ^      ^  ^
                    start  end   start end
            
            学生在每个Latent区域入口:
            - 用当前hidden state的熵决定循环N次
            - 执行N次latent循环
            - 对齐出口位置的teacher hidden state
        """
        ...
        latent_regions = self._identify_latent_regions(
            teacher_entropy, 
            q_lens, 
            ref_answer_position
        )  # List[List[Tuple[start, end]]]
        ...
        for b in range(batch_size):
            ...
            current_pos = q_len  # 当前处理位置
            
            for region_idx, (region_start, region_end) in enumerate(regions):
                stats["total_regions"] += 1
                
                # ------ 3.1 推进到region_start位置（Explicit区域用教师token） ------
                if current_pos < region_start:
                    # 批量处理Explicit tokens
                    explicit_token_ids = ref_input_ids[b:b+1, current_pos:region_start]
                    explicit_embeds = self.get_embd(self.codi, self.model_name)(explicit_token_ids)
                    
                    with torch.no_grad():
                        explicit_outputs = self.codi(
                            inputs_embeds=explicit_embeds,
                            past_key_values=past_key_values,
                            use_cache=True,
                            output_hidden_states=True
                        )
                    past_key_values = explicit_outputs.past_key_values
                    last_hidden = explicit_outputs.hidden_states[-1][:, -1, :]
                
                # ------ 3.2 在Latent入口决定循环次数 ------
                with torch.no_grad():
                    entry_logits = self.codi.lm_head(last_hidden)
                entry_entropy = self.compute_entropy_swir(entry_logits.unsqueeze(1))[:, 0]
                
                teacher_region_len = region_end - region_start
                latent_iters = self._decide_latent_iterations(
                    entry_entropy, 
                    teacher_region_len
                )
                stats["total_latent_iters"] += latent_iters
                
                # ------ 3.3 执行Latent循环 ------
                for iter_idx in range(latent_iters):
                    if self.use_prj:
                        with autocast(dtype=torch.bfloat16):
                    ...
                    last_hidden = latent_outputs.hidden_states[-1][:, -1, :]
                
                # ------ 3.4 对齐Latent出口的hidden state ------
                if region_end < teacher_hidden_states[-1].shape[1]:
                    layers_to_align = range(len(teacher_hidden_states)) if self.adaptive_all_layers else [-1]
                    
                    for layer_idx in layers_to_align:
                        student_h = latent_outputs.hidden_states[layer_idx][:, -1, :]
                        teacher_h = teacher_hidden_states[layer_idx][b:b+1, region_end, :]
                ...
                # 记录详情
                if debug_print and b == 0:
                    stats["region_details"].append({
                        "region": (region_start, region_end),
                        "teacher_len": teacher_region_len,
                        "student_iters": latent_iters,
                        "entry_entropy": entry_entropy[0].item(),
                    })
                
                current_pos = region_end
            
            # ------ 3.5 处理最后一段Explicit（到答案开始位置） ------
            if current_pos < ans_start:
                final_explicit_ids = ref_input_ids[b:b+1, current_pos:ans_start]
                final_explicit_embeds = self.get_embd(self.codi, self.model_name)(final_explicit_ids)
                
                with torch.no_grad():
                    final_outputs = self.codi(
                        inputs_embeds=final_explicit_embeds,
                        past_key_values=past_key_values,
                        use_cache=True,
                        output_hidden_states=True
                    )
                past_key_values = final_outputs.past_key_values
                last_hidden = final_outputs.hidden_states[-1][:, -1, :]
            ...
            if ans_start < teacher_hidden_states[-1].shape[1]:
                ...
                for layer_idx in layers_to_align:
                    student_h = last_hidden if layer_idx == -1 else latent_outputs.hidden_states[layer_idx][:, -1, :]
                    teacher_h = teacher_hidden_states[layer_idx][b:b+1, ans_start, :]
        ...
        total_loss = total_align_loss * self.adaptive_align_loss_factor
        
        # Debug打印
        if debug_print:
            print(f"\n[Adaptive Loss] Regions: {stats['total_regions']}, "
                f"Total Latent Iters: {stats['total_latent_iters']}, "
                f"Align Count: {align_count}, "
                f"Loss: {total_loss.item():.4f}")
            if stats["region_details"]:
                for detail in stats["region_details"][:5]:
                    print(f"  Region {detail['region']}: teacher_len={detail['teacher_len']}, "
                        f"student_iters={detail['student_iters']}, entropy={detail['entry_entropy']:.3f}")
    ...
    def _identify_latent_regions(
        self,
        teacher_entropy: torch.Tensor,  # [B, seq_len]
        q_lens: List[int],
        ref_answer_position: torch.Tensor,
        ...
        使用SwiR逻辑：熵上升进入Latent，熵下降退出
        
        Returns:
            List[List[Tuple[int, int]]]: 每个样本的Latent区域列表 [(start, end), ...]
        """
        batch_size = teacher_entropy.shape[0]
        ...
        for b in range(batch_size):
            ...
            for pos in range(q_len, ans_start):
                cur_entropy = teacher_entropy[b:b+1, pos]
                old_mode = state.mode[0].item()
                state, to_normal, to_soft = self.swir_controller.update(
                    state, cur_entropy, pos - q_len
                )
                new_mode = state.mode[0].item()
                
                is_latent = (new_mode == 0)  # 0 = soft/latent
                
                if is_latent and current_region_start is None:
                    # 进入Latent区域
                    current_region_start = pos
                elif not is_latent and current_region_start is not None:
                    # 离开Latent区域
                    regions.append((current_region_start, pos))
                    current_region_start = None
            
            # 处理末尾未闭合的Latent区域
            if current_region_start is not None:
                regions.append((current_region_start, ans_start))
    ...
    def _decide_latent_iterations(
        self,
        entry_entropy: torch.Tensor,  # [batch_size]
        teacher_region_len: int,
    ) -> int:
        """
        决定Latent循环次数
        
        策略：
        1. 基础次数 = min(num_latent, teacher_region_len)
        2. 可根据熵值微调（熵越高，可能需要更多循环）
        
        Args:
            entry_entropy: 入口位置的熵值
            teacher_region_len: 教师Latent区域的长度
        
        Returns:
            int: 决定的循环次数
        """
        # 基础策略：与教师区域长度相关，但不超过num_latent
        base_iters = min(self.num_latent, max(1, teacher_region_len))
        
        # 可选：根据熵值调整
        # 高熵 -> 更多循环（更复杂的推理）
        # 低熵 -> 更少循环（更简单）
        entropy_val = entry_entropy[0].item()
        
        # 简单的启发式调整
        if entropy_val > 8.0:  # 高熵阈值
            adjusted_iters = min(base_iters + 1, self.num_latent)
        elif entropy_val < 4.0:  # 低熵阈值
            adjusted_iters = max(base_iters - 1, 1)
        else:
            adjusted_iters = base_iters
        
        return adjusted_iters
        ...
        if self.print_loss:
            if self.use_adaptive_loss:
                if self.model_args.use_decoder:
                    ...
                    print(f'loss={loss.item():.4f}, ce_loss={ce_loss_total}, distill_loss={distill_loss_total}, '
                        f'ref_ce_loss={ref_ce_loss}, adaptive_loss={adaptive_loss.item():.4f}')
            else:
                if self.model_args.use_decoder:
