...
def train():
    ...
    # 修改 preprocess 函数 - 移除自动添加 bot_id 和 eot_id 的逻辑
    # 让 model_adaptive.py 中的 forward 函数根据模式动态决定是否添加
    ...
    def preprocess(
        ...
        # ========== 修改：不再自动添加 bot_id ==========
        # 原来: sources_id = [torch.tensor(x.numpy().tolist() + [bot_id], dtype=torch.long) for x in sources_id]
        # 现在: 保持 sources_id 不变，让 model 根据模式动态添加 bot/eot
        # sources_id 保持原样，不添加 bot_id
        
        # ========== 修改：不再自动添加 eot_id 到 answers 开头 ==========
        # 原来: answers_id = [torch.tensor([eot_id] + x.numpy().tolist(), dtype=torch.long) for x in answers_id]
        # 或: answers_id = [torch.tensor([eot_id, tokenizer.eos_token_id] + x.numpy().tolist(), dtype=torch.long) for x in answers_id]
        # 现在: 保持 answers_id 不变
        if training_args.remove_eos:
            # answers_id 已经处理好，不需要额外添加 eot_id
            pass
        else:
            # 只添加 eos_token_id 作为分隔符（如果需要的话），但不添加 eot_id
            # answers_id 已经在上面添加了 eos_token_id，这里不再添加 eot_id
            pass
        ...
        return dict(
            encoder_input_ids=sources_id,  # 不含 bot_id
            decoder_input_ids=answers_id,  # 不含 eot_id 前缀
        ...
        def __init__(self, data_name, raw_data, tokenizer, bot, eot):
            ...
            for num_iter, example in tqdm(enumerate(raw_data)):
                ...
                if "icot" in self.data_name and "full" in self.data_name: # icot-full (GSM8k-Aug-NL)
                    ...
                    # bad data
                    if example["answer"] is None or example["answer"] is None:
                        continue
