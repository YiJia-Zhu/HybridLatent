...
def evaluation(model_args, data_args, training_args):
    ...
    shard_files = sorted(glob.glob(os.path.join(model_args.ckpt_dir, "model-*.safetensors")))
    ...
    elif os.path.exists(os.path.join(model_args.ckpt_dir, "model.safetensors")):
        # 单文件
    ...
    write_json({"ans": ans_pred_list}, f"./results/{data_args.data_name}.json")
    ...
    if model_args.save_ablation:
        save_jsonl_line(f"./results/{data_args.data_name}.jsonl", {'model_name': '-'.join(model_args.ckpt_dir.split('/')[5:]), 'data_name': data_args.data_name, 'soft_weight': model_args.soft_weight, 'acc.': accuracy})
    return 100*accuracy
