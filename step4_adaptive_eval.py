...
def codi_adaptive_generate_with_timing(
    ...
    with torch.no_grad():
        ...
        # ========== 修改：初始不应用 prj，先判断 mode ==========
        # 用未投影的 hidden_states 计算 logits 来判断 mode
        try:
    ...
    for i in range(max_latent_iterations):
        ...
        # ========== 修改：只在 soft mode 循环内应用 prj ==========
        if use_prj and hasattr(model, 'prj'):
            latent_input = model.prj(latent_embd)
        else:
        ...
        with torch.no_grad():
            start_time = time.perf_counter()
            outputs = model.codi(
                inputs_embeds=latent_input,  # 使用投影后的
                use_cache=True,
                output_hidden_states=True,
                past_key_values=past_key_values,
            )
    ...
    # Generate tokens (normal mode) - 后续代码保持不变
    output_emb = eot_emb
...
def main():
    ...
    parser.add_argument("--baseline_mode", type=str, default="random", 
                        choices=["adaptive", "random","all_normal"],
